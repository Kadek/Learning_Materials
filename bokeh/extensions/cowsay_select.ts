...
export class CowsaySelectView extends InputWidgetView {
  model: CowsaySelect
...
export interface CowsaySelect extends CowsaySelect.Attrs {}

export class CowsaySelect extends InputWidget {
  properties: CowsaySelect.Props
  __view_type__: CowsaySelectView

  constructor(attrs?: Partial<CowsaySelect.Attrs>) {
    super(attrs)
  }

  static init_CowsaySelect(): void {
    this.prototype.default_view = CowsaySelectView

    this.define<CowsaySelect.Props>(({String, Array, Tuple, Dict, Or}) => {
      const Options = Array(Or(String, Tuple(String, String)))
      let cow_types = Object.getOwnPropertyNames(cowsay).filter(
        (cow_type) => cow_type.toUpperCase() === cow_type
      );
      return {
        value:   [ String, "" ],
        options: [ Or(Options, Dict(Options)), cow_types ],
      }
    })
  }
}